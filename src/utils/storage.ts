...
export function getStorageKey(key: string) {
  return `${shortName}_${key}`
}

/**
 * @description 存储SessionStorage
 */
export function getStorage({ key, type = 'local' }: { key: string, type?: string }) {
  ...
  if (result && isObject(result)) {
    if (result.expired) {
      const expiredStatus = dayjs().diff(dayjs(result.time), 'ms') >= result.expired
      if (expiredStatus) {
...
export function setStorage({
  ...
  const storageValue = Encrypt(JSON.stringify(result))
